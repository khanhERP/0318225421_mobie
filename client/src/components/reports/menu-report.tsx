...
export function MenuReport({
  ...
  const {
    ...
    queryKey: [
      "https://edpos-mobile-be.onrender.com/api/menu-analysis",
      dateRange.start,
      dateRange.end,
      selectedCategory,
    ],
    queryFn: async () => {
      try {
        ...
        const response = await apiRequest(
          "GET",
          `https://edpos-mobile-be.onrender.com/api/menu-analysis?${params.toString()}`,
        );
        if (!response.ok) {
  ...
  const handleRefresh = () => {
    queryClient.invalidateQueries({ queryKey: ["https://edpos-mobile-be.onrender.com/api/menu-analysis"] });
    refetch();
  };
