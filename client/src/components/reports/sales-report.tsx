...
export function SalesReport() {
    ...
    queryFn: async () => {
      try {
        const response = await fetch(
          `https://09978332-5dc6-4a9a-8375-fec123be89da-00-1qhtnuziydfl4.pike.replit.dev/orders/date-range/${startDate}/${endDate}`,
        );
        if (!response.ok) {
  ...
  const {
    ...
    queryKey: ["https://09978332-5dc6-4a9a-8375-fec123be89da-00-1qhtnuziydfl4.pike.replit.dev/order-items/date-range", startDate, endDate],
    queryFn: async () => {
      try {
        const response = await fetch(
          `https://09978332-5dc6-4a9a-8375-fec123be89da-00-1qhtnuziydfl4.pike.replit.dev/order-items/${startDate}/${endDate}`,
        );
        if (!response.ok) {
          throw new Error(`HTTP error! status: ${response.status}`);
        }
