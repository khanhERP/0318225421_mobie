...
export default function PurchasesPage({ onLogout }: PurchasesPageProps) {
  ...
  const { data: purchaseOrders = [], isLoading: isOrdersLoading } = useQuery<PurchaseOrder[]>({
    queryKey: ["https://edpos-mobile-be.onrender.com/api/purchase-receipts", { startDate, endDate, productFilter, searchTerm }],
    queryFn: async () => {
      ...
      const url = `https://edpos-mobile-be.onrender.com/api/purchase-receipts${params.toString() ? `?${params.toString()}` : ''}`;
  ...
  const bulkDeleteMutation = useMutation({
    mutationFn: async (orderIds: number[]) => {
      return apiRequest("POST", "https://edpos-mobile-be.onrender.com/api/purchase-receipts/bulk-delete", {
        orderIds
      });
    },
    onSuccess: (data) => {
      ...
      queryClient.invalidateQueries({ queryKey: ["https://edpos-mobile-be.onrender.com/api/purchase-receipts"] });
